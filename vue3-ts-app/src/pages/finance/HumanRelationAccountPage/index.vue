...
const relationCards = computed<HumanRelationCardView[]>(() =>
  accounts.value.map((account, index) => {
    ...
    return {
      ...
      noteText: contact?.remark?.trim() || account.remark?.trim() || '点击查看该联系人的全部人情往来记录',
      outgoingTotal,
      incomingTotal,
      netAmount,
      netAmountText: formatSignedCurrency(netAmount),
      recordCount: accountRecords.length,
      latestRecordText: latestRecord ? `最近一笔 ${formatDate(latestRecord.occurredAt)}` : '暂无人情记录',
    }
  }),
)
...
<template>
  <section class="human-relation-account-page" aria-label="人情账户">
    ...
    <header class="human-relation-account-header">
      ...
      <button
        type="button"
        :class="['human-relation-manage-button', { active: isManageMode }]"
    ...
    <template v-else>
      ...
      <section class="human-relation-account-list" aria-label="人情账户列表">
        ...
        >
          ...
          <div class="human-relation-account-card-bottom">
            ...
            <span v-else class="human-relation-account-card-arrow">&gt;</span>
          </div>
        </article>
      </section>
    </template>
